...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (profileErr) throw new Error("Error fetching origin profile: " + profileErr.message);
    if (callerProfile?.role !== 'personal') throw new Error("Only personal trainers can invite students");
    ...
    if (inviteError) {
      ...
      // Let's check via profiles table if user is in our gym DB
      const { data: existingProfiles, error: pErr } = await supabaseAdmin.from('profiles').select('uid, personal_id').eq('email', studentEmail);
      if (existingProfiles && existingProfiles.length > 0) {
         studentAuthId = existingProfiles[0].uid;
         const { error: updErr } = await supabaseAdmin.from('profiles').update({ personal_id: user.id }).eq('uid', studentAuthId);
         if (updErr) throw new Error("Failed to assign existing user: " + updErr.message);
         return new Response(JSON.stringify({ success: true, message: 'Aluno já existia e foi vinculado ao seu plano!' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }});
      }
    ...
    return new Response(JSON.stringify({ error: error.message || 'Erro interno catch edge-function' }), {
      status: 200, 
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
  }
