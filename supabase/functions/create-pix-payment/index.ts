  ...
  try {
    ...
    const mpResponse = await fetch("https://api.mercadopago.com/v1/payments", {
      ...
      body: JSON.stringify({
        ...
        notification_url:
          "https://ueixrbdbtjpyuortrniz.supabase.co/functions/v1/mercadopago-webhook",
      }),
    });
    ...
    if (!mpResponse.ok) {
      ...
      throw new Error(
        `Erro no Mercado Pago: ${mpData.message || "Desconhecido"}`,
      );
    }

    // 4. Calculate end_date based on plan duration
    const now = new Date();
    let endDate = new Date(now);

    if (plan.duration_months && plan.duration_months > 0) {
      endDate.setMonth(endDate.getMonth() + plan.duration_months);
    } else if (plan.duration_days && plan.duration_days > 0) {
      endDate.setDate(endDate.getDate() + plan.duration_days);
    }

    // 5. Update Subscription with External ID and dates
