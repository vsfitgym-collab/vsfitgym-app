...
import { Dumbbell, LineChart, Calendar, ChevronRight, CheckCircle2, Clock, Lock, AlertCircle, ShieldCheck, Star, ArrowRight, Camera } from 'lucide-react';
...
const StudentDashboard: React.FC = () => {
  ...
  const { subscription: planSub, loading: planLoading, isTrial, canSeeDetailedGraphs } = usePlanAccess();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: workoutsData, error: workoutsError } = await supabase
        .from('workouts')
        .select('*')
        .or(`student_id.eq.${profile.uid}${subData?.planId ? `,plan_id.eq.${subData.planId}` : ''}`)
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">Bora treinar, {profile?.name.split(' ')[0]}? 🔥</h1>
          <p className="text-zinc-500">{isTrial ? "Você está no período de teste de 7 dias." : "Seu progresso é constante."}</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          {planSub && (
            <div className={cn(
              "flex items-center gap-2 border px-4 py-2 rounded-xl text-xs font-bold transition-all",
              (planSub.endDate && !isNaN(new Date(planSub.endDate).getTime()) && new Date(planSub.endDate) < new Date())
                ? "bg-red-500/10 border-red-500/20 text-red-500" 
                : "bg-emerald-500/10 border-emerald-500/20 text-emerald-500"
            )}>
              <ShieldCheck className="w-4 h-4" />
              <span>
                 Plano {planSub.plan?.name || '...'}: {(planSub.endDate && !isNaN(new Date(planSub.endDate).getTime())) ? (new Date(planSub.endDate) < new Date() ? 'Expirado' : `Vence em ${format(new Date(planSub.endDate), "dd/MM")}`) : 'Status desconhecido'}
              </span>
            </div>
          )}
          <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 px-4 py-2 rounded-xl">
            <Calendar className="w-5 h-5 text-orange-500" />
            <span className="text-zinc-100 font-bold">{format(new Date(), "EEEE, d 'de' MMMM", { locale: ptBR })}</span>
          </div>
        </div>
      </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Star className={cn("w-5 h-5", planSub && new Date(planSub.endDate) > new Date() ? "text-orange-500" : "text-white")} />
              <h2 className={cn("text-xl font-black uppercase tracking-widest", planSub && new Date(planSub.endDate) > new Date() ? "text-white" : "text-white")}>
                {planSub && new Date(planSub.endDate) > new Date() ? "Você é Premium! ⭐" : "Experimente o Premium"}
              </h2>
            </div>
            <p className={cn("text-sm font-medium max-w-xl", planSub && new Date(planSub.endDate) > new Date() ? "text-zinc-400" : "text-white/90")}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-zinc-500 text-sm font-bold uppercase">Treinos Ativos</span>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-zinc-500 text-sm font-bold uppercase">Último Peso</span>
          </div>
          <p className="text-4xl font-black text-white">{lastEvolution?.weight || '--'} <span className="text-lg font-bold text-zinc-500">kg</span></p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-zinc-500 text-sm font-bold uppercase">Fotos de Progresso</span>
          </div>
          {isTrial ? (
            ...
            <p className="text-4xl font-black text-white">{lastEvolution?.photos?.length || 0}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {workouts.length > 0 ? workouts.map((workout) => {
            const isSubActive = planSub && new Date(planSub.endDate) >= new Date();
            ...
            return (
              <button 
                ...
                onClick={() => {
                  if (isLocked) {
                    alert('Este é um treino Premium! Renove sua assinatura com seu personal para acessar.');
                  } else {
                ...
                className={cn(
                  ...
                  isLocked ? "border-zinc-800 opacity-60 grayscale cursor-not-allowed" : "border-zinc-800 hover:border-orange-600/50"
                )}
              >
                <div className="flex items-center gap-4">
                  <div className={cn(
                    "w-14 h-14 bg-zinc-950 rounded-2xl flex items-center justify-center border transition-colors",
                    isLocked ? "border-zinc-800" : "border-zinc-800 group-hover:border-orange-600/30"
                  )}>
                    {isLocked ? <Lock className="w-7 h-7 text-zinc-700" /> : <Dumbbell className="w-7 h-7 text-orange-500" />}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-zinc-500">{workout.exercises.length} exercícios</p>
                  </div>
                </div>
                {isLocked ? (
            ...
            <div className="col-span-full bg-zinc-900 border border-zinc-800 p-12 rounded-3xl text-center">
              ...
              <p className="text-zinc-500">Você ainda não tem treinos cadastrados.</p>
              <p className="text-xs text-zinc-600 mt-1">Fale com seu personal trainer.</p>
            </div>
          )}
        </div>
      </div>
